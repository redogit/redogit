...
## 41. Nonclash-clique inclusion-exclusion carrier

Section 40 used a logarithmic vertex cover of the clause nonclash graph to isolate a hitting core.

The full inclusion-exclusion anatomy is stronger and gives an exact terminal whenever the nonclash graph itself has only polynomially many cliques.

Let F be a canonical CNF with tautological clauses already removed and no empty clause. Let n be the number of variables.

For each clause C define the falsifying subcube

    Bad(C)
      =
    { truth assignments that falsify every literal of C }.

Define the nonclash graph Q_F:

- one vertex for every clause;
- edge {C,D} exactly when C and D contain no complementary literal pair.

Equivalently, for A=M(F)^T, Q_F is the conformality graph of the clause columns.

### 41.1 Clique-intersection theorem

For a nonempty clause set T:

    intersection_{C in T} Bad(C) is nonempty
...
If two clauses clash on x / not-x, falsifying both would require x to be both FALSE and TRUE, so their falsifying subcubes are disjoint.

Conversely, if all clause pairs are nonclashing, the union of all literal-falsification requirements contains no variable in opposite required states. Assign every appearing variable the value that falsifies its literal. The remaining variables are free.

Thus pairwise nonclash is already global consistency for simultaneous clause falsification.

For a clique T, let

    V(T)

be the set of variables occurring in its clauses.

Then:

    | intersection_{C in T} Bad(C) |
      =
    2^(n-|V(T)|).

### 41.2 Exact model-count formula

By inclusion-exclusion over the union of clause-falsification events:

    #SAT(F)
      =
    2^n
      +
    sum over nonempty cliques T of Q_F
        (-1)^|T| 2^(n-|V(T)|).

Only cliques contribute because every nonclique intersection is empty.

This is an exact integer identity.

The hitting formula from Section 40 is the special case:

    Q_F has no edges,

so the only nonempty cliques are singleton clauses and the formula reduces to

    #SAT(F)
      =
    2^n - sum_C 2^(n-|C|).

### 41.3 Polynomial envelope from degeneracy

Let d be the degeneracy of Q_F.

Compute a degeneracy ordering

    v_1,...,v_m

such that every clause vertex has at most d later nonclash neighbors.

Every nonempty clique has a unique earliest vertex v_i in this ordering.

All other members of that clique lie inside the at-most-d later-neighbor set of v_i.

Therefore all cliques can be generated by testing at most

    m 2^d

candidate subsets.

No output-sensitive assumption is hidden here; this is a direct enumeration bound.

Hence for any fixed constant c:
...
Each clique contribution uses only:

- its clause IDs;
- the union of its variable IDs;
- a sign determined by clique cardinality;
- an O(n)-bit power-of-two count.

So exact #SAT and SAT decision run in polynomial total work under the logarithmic-degeneracy guard.

This strictly contains the hitting case d=0.

A logarithmic vertex-cover bound from Section 40 also implies a logarithmic degeneracy bound, so the present carrier subsumes that terminal as an execution option.

### 41.4 Homeward witness without guard decay

Naive self-reduction would appear dangerous because simplifying F after assigning a variable can destroy clause clashes, add edges to a newly recomputed nonclash graph, and increase its degeneracy.

Do not recompute the carrier.

Enumerate the clique set of the original Q_F once.

For a partial assignment alpha, count satisfying extensions of alpha by intersecting the **original** falsifying subcubes with alpha.

For every original nonempty clique T:

1. derive the unique partial assignment beta_T that falsifies every clause in T;
2. if beta_T conflicts with alpha, its intersection contribution is zero;
3. otherwise:

       |Ext(alpha) intersect intersection_{C in T} Bad(C)|
         =
       2^(n-|dom(alpha) union dom(beta_T)|).

Thus:

    #SAT(F | alpha)

is computed using the same original polynomial clique list.

No new conformal/nonclash edges are introduced into the proof carrier.

If the count is positive, reconstruct a witness in stable variable-ID order:

    try x=0;
    compute extension count from original clique carrier;
    if positive keep 0;
    otherwise set x=1.

At most n calls are needed.

Therefore Homeward witness reconstruction remains polynomial under exactly the original degeneracy guard.

### 41.5 Negative certificate

For an UNSAT result retain:

- Q_F;
- the degeneracy ordering;
- every enumerated clique T;
- its source clause IDs;
- V(T);
- the signed exact integer contribution;
- the final sum #SAT(F)=0.

Verification checks:

1. every listed T is a clique;
2. every clique has its unique earliest vertex and is included exactly once;
3. every candidate subset from later-neighbor sets was tested;
4. every power-of-two contribution is correct;
5. the exact integer total is zero.

All arithmetic uses polynomial-bit integers.

### 41.6 One-degree / anti-decay interpretation

An edge of Q_F is not a Boolean branch.

It records one **conformal overlap degree**:

    two clause-falsification events may coexist.

The degeneracy orientation gives every clause at most d forward overlap coordinates.

For d=O(log N_input), all jointly compatible overlap events can be retained exactly rather than approximated or discarded.

The carrier therefore moves away from Knowledge Decay by keeping the complete overlap relation inside a polynomial envelope.

### 41.7 Bounded differential validation

Two separate bounded checks were run.

#### Count identity

10,000 random small CNFs were compared between:

    brute-force truth-table #SAT

and

    degeneracy-ordered nonclash-clique inclusion-exclusion.
...
5,000 additional random CNFs were solved using the original clique carrier for every partial-assignment extension count.

Results:

    tested: 5,000
    witness/count mismatches: 0.

These panels are implementation evidence only. The clique-intersection and inclusion-exclusion proofs above are the theorem.

### 41.8 Updated qualitative-matrix terminal

The exact terminal is now:

    LOG_NONCLASH_DEGENERACY
        ->
    ENUMERATE ALL CONFORMAL CLAUSE CLIQUES
        ->
    EXACT INCLUSION-EXCLUSION #SAT
        ->
    SAT / UNSAT + HOMEWARD.

HITTING / TIGHT is the zero-degeneracy endpoint.

LOG_CONFORMAL_VERTEX_COVER is a sufficient but no longer necessary special case.

The remaining qualitative-matrix residual must therefore have:

    superlogarithmic nonclash / conformality degeneracy
