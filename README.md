...
The coupling parameter has now been generalized beyond vertex cover. For a current cohort S, form the Dean coupling graph J_S by joining candidate-conflict edges with candidate-to-S support edges. A candidate-stable set B has augmentation deficiency |B|-|N(B)∩S|. This objective has an exact 2^O(w) dynamic program on a tree decomposition of J_S of width w: candidate bits enforce conflicts, support bits record which current students would be displaced, and the objective is charged only when vertices leave the bag. If the optimum is positive, inclusion-minimization produces an exact +1 augmentation; if the optimum is zero, S is maximum. With current single-exponential treewidth decomposition algorithms, treewidth(J_S)=O(log n) therefore gives a deterministic polynomial signed augment-or-certify carrier without any binary/graphic assumption.
