...
A second scoped carrier handles that coupling when the conflict graph has a vertex cover Z of size O(log n): enumerate only the signed include/exclude choices on Z, remove the minimal conflict neighbors implied by each positive choice, and solve the resulting marked-cycle problem on the series-parallel support graph by bounded-interface dynamic programming. This remains exact and polynomial in the stated logarithmic-cover regime.
